...
export function MotionDnDProvider({ children, ...context }: PropsWithChildren<MotionDnDContextType>) {
  const valueRef = useRef<MotionDnDContextType | null>(null);
  if (valueRef.current === null) {
    valueRef.current = context;
  }
  return <MotionDnDContext.Provider value={valueRef.current}>{children}</MotionDnDContext.Provider>;
}
